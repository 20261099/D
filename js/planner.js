...
class PlannerManager {
  ...
  _renderTextbooks() {
    ...
    container.innerHTML = tbs.map(tb => {
      ...
      return `
        <div class="tb-item">
          ...
          <button class="tb-re-btn" data-id="${tb.id}" data-name="${tb.subjectName}" title="재등록">↺</button>
    ...
    // 재등록 버튼
    container.querySelectorAll('.tb-re-btn').forEach(btn => {
      btn.onclick = () => {
        if (typeof AppState !== 'undefined') {
          AppState._reregisterTargetId   = btn.dataset.id;
          AppState._reregisterTargetName = btn.dataset.name;
        }
        if (typeof navigate !== 'undefined') navigate('textbook');
        else document.getElementById('screen-textbook')?.classList.remove('hidden');
      };
    });
  }
