...
// ── 개발자 비밀 코드 (배포 후에도 숨김) ──────────────────
// 키보드로 "fitdev" 입력하면 수룡이 점수 추가 창 뜸
(function() {
  const SECRET = 'fitdev';
  let buf = '';
...
function _showDevPointsModal() {
  const amounts = [1, 3, 5, 10];
  const el = document.createElement('div');
  el.id = 'dev-modal';
  el.style.cssText = 'position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.6);display:flex;align-items:center;justify-content:center';
  el.innerHTML = `
    <div style="background:#fff;border-radius:16px;padding:24px 28px;text-align:center;min-width:240px">
      <div style="font-size:1.1rem;font-weight:800;margin-bottom:12px">🔑 개발자 모드</div>
      <div style="font-size:.85rem;color:#888;margin-bottom:16px">수룡이 점수 추가</div>
      <div style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap">
        ${amounts.map(n => `<button onclick="_devAddPoints(${n})"
          style="padding:10px 16px;border:none;border-radius:8px;background:#7b6fc4;color:#fff;cursor:pointer;font-weight:700">+${n}점</button>`).join('')}
      </div>
      <button onclick="document.getElementById('dev-modal').remove()"
        style="margin-top:16px;padding:8px 20px;border:1px solid #ddd;border-radius:8px;background:#fff;cursor:pointer">닫기</button>
    </div>`;
  document.body.appendChild(el);
  el.onclick = e => { if (e.target === el) el.remove(); };
}

async function _devAddPoints(n) {
  document.getElementById('dev-modal')?.remove();
  if (typeof Suryong !== 'undefined') {
    await Suryong.addPoints(n);
    SuryongRoom.refresh();
  }
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // 로딩 최대 대기: 인증이 해결됐으면 로그인 화면을 강제로 보여주지 않음
  // 인증 미해결 상태로 5초 경과 시에만 로그인 화면으로 이동 (네트워크 문제 대비)
  setTimeout(() => {
    if (!_authResolved && AppState.currentScreen === 'screen-loading') {
      showScreen('screen-auth');
    }
  }, 5000);
});
...
function navigate(target) {
  ...
  switch (target) {
    ...
    case 'playground':
      showScreen('screen-playground');
      if (Auth.currentUser) {
        document.getElementById('pg-guest-notice')?.classList.add('hidden');
        document.getElementById('pg-actions')?.classList.remove('hidden');
        PlaygroundUI.enterScreen();
      } else {
        document.getElementById('pg-guest-notice')?.classList.remove('hidden');
        document.getElementById('pg-actions')?.classList.add('hidden');
      }
      break;
...
async function initApp(user) {
  ...
  try {
    ...
    if (typeof ProfileManager !== 'undefined') {
      ...
      ProfileUI.initAccountCard();
    }
    // 내 공개 프로필 동기화 (친구에게 공부 시간이 보이도록)
  ...
  if (granted) {
    ...
    try {
      ...
      Tracker.onResults(() => {
        ...
        // 일시정지 중엔 깜빡임·표정 측정 안 함 (기준값 왜곡 방지)
        if (mode === 'ai' && Tracker.isCalibrated() && !Timer.isPaused())
          BlinkEngine.tick(Tracker.isEyeClosed());
        if (mode === 'ai' && !Timer.isPaused())
          ImmersionEngine.update(Tracker.getExpression());
...
function watchCalibration() {
  // AI/설정 타이머 모두 동일한 calib-overlay 사용
  const ov = document.getElementById('calib-overlay');
  const bar = document.getElementById('calib-bar');
  ...
  const poll = setInterval(() => {
    const prog = Tracker.getCalibProgress ? Tracker.getCalibProgress() : 0;
    if (bar) bar.style.width = (prog * 100) + '%';
    if (Tracker.isCalibrated()) {
      clearInterval(poll);
      if (ov) ov.classList.add('hidden');
      setEyeStatus('👁 집중 중', 'open');
    }
  }, 150);
}
...
function updateBlinkStats() {
  ...
  // 일시정지 중: 표시 동결 (값 계속 바뀌면 혼란)
  if (Timer.isPaused()) {
    const pauseBadge = document.getElementById('bs-rate');
    if (pauseBadge && !pauseBadge.textContent.includes('⏸')) {
      pauseBadge.textContent = '⏸ 일시정지';
    }
    const exprEl = document.getElementById('bs-expr');
    if (exprEl) exprEl.textContent = '⏸';
    return;
  }

  const count   = BlinkEngine.windowBlinks;
  // 일시정지 보정된 windowStart 사용 (blink.js의 pause/resume에서 보정됨)
...
function onPhaseChange(phase, sMin, rMin) {
  if (phase === 'break') {
    ...
    Planner.endSession().then(() => {
      ...
      // 세션 종료 후 공개 프로필 동기화 (친구에게 최신 공부 시간 반영)
      if (typeof PlaygroundManager !== 'undefined') {
        PlaygroundManager.syncPublicProfile().catch(() => {});
      }
    }).catch(() => {});
...
function setupTimerControls() {
  document.getElementById('btn-pause-resume').onclick = () => {
    ...
    const aiMode = AppState.settings?.timerMode === 'ai';
    if (Timer.isPaused()) {
      Timer.resume();
      btn.textContent = '일시정지';
      // 재개 시 공부 페이즈면 졸음·깜빡임·몰입 재시작
      if (Timer.getPhase() === 'study') {
        DrowsyDetector.start();
        DrowsyDetector.onDrowsy(() => onDrowsy());
        DrowsyDetector.onAwake(() => onAwake());
        if (aiMode) { BlinkEngine.resume(); ImmersionEngine.start(); }
      }
    } else {
      Timer.pause();
      btn.textContent = '▶ 재개';
      // 일시정지 시 졸음·깜빡임·몰입 감지 완전 중단
      DrowsyDetector.stop();
      Alarm.stopDrowsinessAlarm();
      document.getElementById('drowsy-banner')?.classList.add('hidden');
      if (aiMode) { BlinkEngine.pause(); ImmersionEngine.stop(); }
    }
  };
  document.getElementById('btn-skip').onclick = () => {
    const wasPaused = Timer.isPaused();
    Timer.skipPhase(); // 페이즈 전환 (UI는 onPhaseChange로 갱신)
    if (wasPaused) {
      // 전환 후 다시 일시정지 상태 강제 유지
      DrowsyDetector.stop();
      Alarm.stopDrowsinessAlarm();
      if (AppState.settings?.timerMode === 'ai') {
        BlinkEngine.pause();
        ImmersionEngine.stop();
      }
      // 일시정지 중이라 tick이 없으므로 새 페이즈 시간을 화면에 즉시 표시
      Timer.manualUpdateUI();
      // 버튼 텍스트 재개 유지
      const btn = document.getElementById('btn-pause-resume');
  ...
  document.getElementById('btn-end-session').onclick = async () => {
    ...
    if (typeof PlaygroundManager !== 'undefined') PlaygroundManager.syncPublicProfile().catch(() => {});
    if (cur2) {
...
document.getElementById('btn-capture')?.addEventListener('click', () => {
  ...
  // 재등록 모드: 이름 미리 채움
  if (AppState._reregisterTargetName) {
    const inp = document.getElementById('reg-subject-input');
    if (inp) inp.value = AppState._reregisterTargetName;
  }
...
document.getElementById('btn-reg-save')?.addEventListener('click', async () => {
  ...
  try {
    ...
    // 재등록 모드: id/name 재사용
  const isReReg = !!AppState._reregisterTargetId;
  const histogram = TextbookMgr.extractHistogram(
    ...
    let color = TextbookMgr.getColorForSubject(name);
    if (isReReg) {
      const existingId   = AppState._reregisterTargetId;
      const originalName = AppState._reregisterTargetName;
      const existing     = TextbookMgr.textbooks.find(t => t.id === existingId);
      if (existing) {
        const nameChanged = name && name !== existing.subjectName;

        existing.histogram   = histogram;
        existing.thumbnail   = thumbnail;
        if (name) existing.subjectName = name; // 이름 변경 반영
        await Storage.saveTextbook(existing);

        // 이름이 바뀌었으면 → 관련 기록 전체 업데이트
        if (nameChanged) {
          // 1. Planner 세션 기록
          (Planner.sessions || []).forEach(s => {
            if (s.textbookId === existingId) s.subjectName = name;
          });
          await Storage.saveStudySessions(Planner.sessions).catch(() => {});

          // 2. 복습 스케줄 (review.js)
          if (typeof Review !== 'undefined') {
            Review.schedules.forEach(r => {
              if (r.textbookId === existingId) r.subjectName = name;
            });
            await Review._save().catch(() => {});
          }

          console.info(`[App] 교재 이름 변경: "${originalName}" → "${name}" — 기록 일괄 업데이트`);
        }
      }
      AppState._reregisterTargetId   = null;
      AppState._reregisterTargetName = null;
    } else {
