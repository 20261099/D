...
class SuryongManager {
  constructor() {
    this.state = {
      stage: 0,
      points: 0,
      highestStage: 0,      // 역대 최고 단계 (졸업해도 유지)
  ...
  async _evolve() {
    ...
    if (nextStage > 5) {
      ...
      // 역대 최고 단계 갱신 (졸업 후에도 해금 유지)
      if (nextStage > (this.state.highestStage || 0)) this.state.highestStage = nextStage;
      this.state.points = 0;
      await this.save();
  ...
  _renderCollections() {
    ...
    if (grid) {
      if (!colls.length) {
        ...
        grid.innerHTML = colls.map((dept, idx) => `
          <div class="coll-item" title="${dept.name}"
            onclick="SuryongRoom.openCollectionDetail(${idx})">
            <img src="${getHakgwaImage(dept.id)}" alt="${dept.name}">
            <div class="coll-name">${dept.name}</div>
            <div class="coll-playground-badge" style="display:${dept.showInPlayground!==false?'block':'none'}">🌸</div>
          </div>`).join('');
      }
    }
...
// ── 컬렉션 상세 (메시지 + 놀이터 공유) ──────────────────────────
SuryongRoom.openCollectionDetail = function(idx) {
  const colls = Suryong.state.collections;
  const dept  = colls[idx];
  if (!dept) return;

  const modal = document.getElementById('coll-detail-modal');
  if (!modal) return;
  modal.dataset.idx = idx;

  // 이미지
  const img = modal.querySelector('.cd-char img');
  if (img) img.src = getHakgwaImage(dept.id);
  // 이름
  const name = modal.querySelector('.cd-name');
  if (name) name.textContent = dept.name + ' 수룡이';
  // 메시지
  const msgInput = modal.querySelector('.cd-message-input');
  if (msgInput) msgInput.value = dept.message || '';
  // 놀이터 공유 토글
  const toggle = modal.querySelector('.cd-playground-toggle');
  if (toggle) toggle.checked = dept.showInPlayground !== false;

  modal.classList.remove('hidden');
};

SuryongRoom.saveCollectionDetail = async function() {
  const modal = document.getElementById('coll-detail-modal');
  const idx   = Number(modal.dataset.idx);
  const dept  = Suryong.state.collections[idx];
  if (!dept) return;

  const msg    = modal.querySelector('.cd-message-input').value.trim();
  const shared = modal.querySelector('.cd-playground-toggle').checked;
  dept.message          = msg || '안녕! 나 ' + dept.name + ' 수룡이야 🌸';
  dept.showInPlayground = shared;
