 ...
 * [병합] 타이머1 핵심 수정사항: 과거 누적평균 → 그날 첫 BASELINE_WINDOW_SEC(5분) 기준으로 전환
 * [병합] 타이머1 핵심 수정사항: ratio*10분 직접 연장 → Blink Score + 몰입 보너스 → (점수/100)*10분 방식으로 전환
 */
...
class BlinkAnalyzer {
  constructor() {
    ...
    this.immersionCount = 0;      // 윈도우 내 몰입 횟수
  ...
  pause() {
    ...
    this.paused     = true;
    this._pausedAt  = Date.now(); // 일시정지 시작 시각 기록
  }
  resume() {
    if (!this.paused) return;
    this.paused = false;
    if (this._pausedAt && this.windowStart) {
      // 일시정지 동안 흐른 시간을 windowStart에 더해 보정
      // → resume 후 경과시간 계산에 일시정지 시간이 포함되지 않음
      const pausedMs   = Date.now() - this._pausedAt;
      this.windowStart += pausedMs;
    }
    this._pausedAt = null;
  }
  ...
  // 실시간 집중 상태 판정 (immersion.js에서 사용)
  isFocused() {
