...
const AlbumDB = {
  ...
  _open() {
    ...
    return new Promise((resolve, reject) => {
      const req = indexedDB.open('fittimer_album', 1);
      req.onupgradeneeded = () => {
        const db = req.result;
...
const AlbumUI = {
  ...
  async renderGallery() {
    ...
    await AlbumDB.cleanupOld().catch(() => {});
    const dates = await AlbumDB.getDates().catch(() => []);
    ...
    const cards = await Promise.all(dates.map(d => this._buildCard(d)));
  ...
  async openPlayer(date) {
    ...
    try {
      const result = await getOrEncodeVideo(date, (cur, total) => {
