...
class ReviewManager {
  ...
  async handleInitialResponse(choice) {
    // choice: 'only_review' | 'review_and_study' | 'skip' | 'later'
    const { reviewId, textbookId, subjectName } = this._session;

    switch (choice) {
      case 'only_review':
        // "쭉 그것만 했어요" → 다음 쉬는시간에 done_ask
        this._session.phase = 'done';
        break;

      case 'review_and_study':
        // "복습도 하고 공부도 했어요" → 완료 + 오늘 기준 새 주기
        await this._markDone(reviewId);
        await this._newCycleFromToday(textbookId, subjectName);
        await this._completeMissionM3();
        this._resetSession();
        break;

      case 'skip':
        // "안할 예정이에요" → 이 세션 동안 이 교재 팝업 안 띄움
        this._session.skipIds.add(textbookId);
        this._resetSession();
        break;

      case 'later':
        // "이따가 할거에요" → 다음 쉬는시간에 다시 initial_ask
        this._session.phase = 'initial';
        // reviewId 유지, 다음 break 때 다시 묻기
        this._session.reviewId = null; // checkBreak에서 다시 찾도록
        break;
    }
  ...
  async handleDoneResponse(choice) {
    // choice: 'done_and_studied' | 'done_only' | 'not_yet'
    const { reviewId, textbookId, subjectName } = this._session;

    switch (choice) {
      case 'done_and_studied':
        // "끝나고 진도도 나갔어요" → 완료 + 새 주기
        await this._markDone(reviewId);
        await this._newCycleFromToday(textbookId, subjectName);
        await this._completeMissionM3();
        this._resetSession();
  ...
  async _savePushSub(sub) {
    ...
    console.info('[Review] Push 구독 저장 완료');
  }
