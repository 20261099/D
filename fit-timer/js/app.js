...
async function startTimerScreen(mode) {
  ...
  if (granted) {
    ...
    Tracker.onResults(() => {
      ...
      if (mode === 'ai') ImmersionEngine.update(Tracker.getExpression());   // ★ 이 줄 추가
  ...
  if (mode === 'ai') ImmersionEngine.start();   // ★ 이 줄 추가
...
function setupTimerControls() {
  document.getElementById('btn-pause-resume').onclick = () => {
    ...
    if (Timer.isPaused()) { Timer.resume(); btn.textContent = '일시정지'; }
    else                  { Timer.pause();  btn.textContent = '▶ 재개'; }
  };
