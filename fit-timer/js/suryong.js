...
const DEPARTMENTS = [
  ...
  { id: 10, name: '동양학과',                college: '문과' },
  { id: 11, name: '디자인과',                college: '예술' },
  { id: 12, name: '무용예술학과',            college: '예술' },
  { id: 13, name: '문화예술경영학과',        college: '경영' },
  { id: 14, name: '미디어영상연기학과',      college: '예술' },
  { id: 15, name: '미디어커뮤니케이션학과',  college: '사회' },
  { id: 16, name: '바이오식품공학과',        college: '이과' },
  { id: 17, name: '바이오신약의과학부',      college: '이과' },
  { id: 18, name: '바이오헬스융합학부',      college: '이과' },
  { id: 19, name: '법학부',                  college: '법과' },
  { id: 20, name: '뷰티산업학과',            college: '보건' },
  { id: 21, name: '사학과',                  college: '문과' },
  { id: 22, name: '사회교육과',              college: '사범' },
  { id: 23, name: '사회복지학과',            college: '사회' },
  { id: 24, name: '서비스디자인공학과',      college: 'IT'  },
  { id: 25, name: '서양학과',                college: '문과' },
  { id: 26, name: '성악과',                  college: '음악' },
  { id: 27, name: '소비자산업학과',          college: '경영' },
  { id: 28, name: '수리통계데이터사이언스학부', college: '이과' },
  { id: 29, name: '스포츠과학부',            college: '보건' },
  { id: 30, name: '심리학과',                college: '사회' },
  { id: 31, name: '영어영문학과',            college: '문과' },
  { id: 32, name: '유아교육과',              college: '사범' },
  { id: 33, name: '윤리교육과',              college: '사범' },
  { id: 34, name: '융합보안공학과',          college: 'IT'  },
  ...
  { id: 42, name: '창의융합학부',            college: 'IT'  },
  { id: 43, name: '청정신소재공학과',        college: '이과' },
  { id: 44, name: '컴퓨터공학과',            college: 'IT'  },
  { id: 45, name: '프랑스어문&문화학과',     college: '문과' },
  { id: 46, name: '한문교육과',              college: '사범' },
  { id: 47, name: '현대실용음악학과',        college: '음악' },
  { id: 48, name: '화학&에너지융합학부',     college: '이과' },
];

function getHakgwaImage(deptId) {
  const dept = DEPARTMENTS.find(d => d.id === deptId);
  return `images/hakgwa/${dept.name}.png`;
}

// ── 수룡이 단계 ────────────────────────────────────────────────
const SURYONG_STAGES = [
  { id: 0, name: '알',           image: 'images/suryong/알.png',  pointsNeeded: 3,  desc: '처음엔 작은 알이에요 🥚' },
  { id: 1, name: '꼬물이 수룡이', image: 'images/suryong/꼬물이.png', pointsNeeded: 10, desc: '꼬물꼬물 자라고 있어요 🐣' },
  { id: 2, name: '유치원 수룡이', image: 'images/suryong/유치원생.png', pointsNeeded: 15, desc: '첫 등원! 모자가 귀엽죠? 🎓' },
  { id: 3, name: '학생 수룡이',  image: 'images/suryong/학생.png',  pointsNeeded: 20, desc: '열심히 공부 중이에요 📚' },
  { id: 4, name: '과잠 수룡이',  image: null,                       pointsNeeded: 25, desc: '드디어 과잠을 입었어요! 🎽' },
  { id: 5, name: '학사모 수룡이', image: 'images/suryong/학사모최종.png', pointsNeeded: 30, desc: '졸업이 눈앞이에요! 🎓' },
];
...
class SuryongManager {
  ...
  return SURYONG_STAGES[stage]?.image || 'images/suryong/su1.svg';
}
...
const SuryongRoom = {
  ...
  _renderMissions() {
    ...
    MISSIONS.forEach(m => {
      ...
      let btnHtml = '';
      if (disabled) {
        btnHtml = '<span class="mission-soon">준비중</span>';
      } else if (claimed) {
        btnHtml = '<span class="mission-claimed">✅ 받음</span>';
      } else if (done) {
        btnHtml = `<button class="mission-claim-btn" onclick="Suryong.claimMission('${m.id}')">🎁 점수 받기</button>`;
      } else {
        btnHtml = `<span class="mission-reward">+${m.points}점</span>`;
      }
      ...
      html += `
        <div class="mission-item ${done && !claimed ? 'done-unclaimed' : ''} ${claimed ? 'done' : ''} ${disabled ? 'disabled' : ''}">
          ...
          <div class="mission-pts">${btnHtml}</div>
        </div>`;
    });
  ...
  _renderDragon() {
    ...
    if (Suryong.state.stage === 4 && Suryong.state.department) {
      stageName = Suryong.state.department.name.slice(0,4) + ' 수룡이';
    }
      ...
      if (!colls.length) {
        ...
        grid.innerHTML = colls.map(dept => `
          <div class="coll-item" title="${dept.name}">
            <img src="${getHakgwaImage(dept.id)}" alt="${dept.name}"> // ← 교체
            <div class="coll-name">${dept.name.slice(0,4)}</div>
          </div>`).join('');
      }
    }
